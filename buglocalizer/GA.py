...
class GA:
    def __init__(self, 
        ...
        elif (mutation_type == "two"):
            self.mutation = self.Twors
        elif (mutation_type == "cim"):
            self.mutation = self.CIM
        elif (mutation_type == "psm"):
            self.mutation = self.PSM
        ...
        if not (callback_generation is None):
            ...
            if callable(callback_generation):
                ...
                if (callback_generation.__code__.co_argcount == 1):
                    ...
                    print("Starting from GA, the callback_generation parameter is deprecated and will be removed in a later release of GA. Please use the on_generation parameter instead.")
                else:
                    self.valid_parameters = False
    ...
    def CIM(self, offspring):
        for idx in range(offspring.shape[0]):
            p = random.randint(0, len(offspring[idx]) - 1)
            # print(p)
            # print(offspring[idx, 0:p])
            # a = [1, 2, 3]
            # reversed(a)
            # print(list(reversed(a)))
            offspring[idx, 0:p] = list(reversed(offspring[idx, 0:p]))
            offspring[idx, p:] = list(reversed(offspring[idx, p:]))
    ...
    def plot_result(self, title="GA - Iteration vs. Fitness", xlabel="Generation", ylabel="Fitness", linewidth=3):
